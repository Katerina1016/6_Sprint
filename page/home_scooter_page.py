...
class HomePage(MainPage):
    ...
    def yandex_logo_click(self):
        self.click_on_button(BasePageLocators.logo_yandex)

    @allure.step('Клик на слово Самокат')
    def scooter_logo_click(self):
        self.click_on_button(BasePageLocators.logo_scooter)

    @allure.step('Клик на кнопку Заказать в шапке')
    def order_button_click(self):
        self.click_on_button(BasePageLocators.order_button_from_header)

    @allure.step('Проверка текста в шапке')
    def check_main_title(self):
        return self.wait_about_element_located(BasePageLocators.scooter_logo_text).is_displayed()

    @allure.step('Клик на кнопку Принять куки')
    def accept_cookie(self):
        self.wait_about_element_located(BasePageLocators.accept_cookie_button)
        self.click_on_button(BasePageLocators.accept_cookie_button)

    @allure.step('Клик на кнопку заказать внизу страницы')
    def order_from_button_bottom_page(self):
        self.scroll_to_locator(BasePageLocators.order_button_from_page)
        self.check_element_displayed(BasePageLocators.order_button_from_page)
        self.click_on_button(BasePageLocators.order_button_from_page)

    @allure.step('Скролл до вопросов')
    def scroll_to_questions(self):
        self.scroll_to_locator(BasePageLocators.drop_down_lists)
